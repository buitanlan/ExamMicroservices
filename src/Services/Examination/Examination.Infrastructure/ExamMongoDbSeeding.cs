...
public static class ExamMongoDbSeeding
{
    public static async Task SeedAsync(IMongoClient mongoClient, IOptions<ExamSettings> settings)
    {
        var policy = CreatePolicy( nameof(ExamMongoDbSeeding));
                ...
                0)
            {
                await database.GetCollection<Category>(Constants.Collections.Category)
                    .InsertManyAsync(new List<Category>
                    {
                        ...
                        new(categoryId3, "Category 3", "category-3"),
                        new(categoryId4, "Category 4", "category-4"),
                    });
            }
    ...
    private static AsyncRetryPolicy CreatePolicy(string prefix, int retries = 3)
    {
